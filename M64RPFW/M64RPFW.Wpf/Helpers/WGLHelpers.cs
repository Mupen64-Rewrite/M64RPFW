...
{
    internal static class WGLHelpers
    {
        ...
        public static (int[] pixFmt, int[] context) GenAttributeLists(Dictionary<GLAttribute, int> attrs)
        {
            List<int> pixFmtAttrs = new()
            {
                WGL_DRAW_TO_WINDOW_ARB, 1,
                WGL_STENCIL_BITS_ARB, 8,
                WGL_SUPPORT_OPENGL_ARB, 1
            };
            List<int> contextAttrs = new();

            if (attrs.TryGetValue(GLAttribute.DoubleBuffer, out int value))
                pixFmtAttrs.AddRange(new[] { WGL_DOUBLE_BUFFER_ARB, value });
            else
                pixFmtAttrs.AddRange(new[] { WGL_DOUBLE_BUFFER_ARB, 1 });

            if (attrs.TryGetValue(GLAttribute.BufferSize, out value))
                pixFmtAttrs.AddRange(new[] { WGL_COLOR_BITS_ARB, value });
            else
                pixFmtAttrs.AddRange(new[] { WGL_COLOR_BITS_ARB, 32 });

            if (attrs.TryGetValue(GLAttribute.DepthSize, out value))
                pixFmtAttrs.AddRange(new[] { WGL_DEPTH_BITS_ARB, value });
            else
                pixFmtAttrs.AddRange(new[] { WGL_DEPTH_BITS_ARB, 24 });

            if (attrs.TryGetValue(GLAttribute.RedSize, out value))
                pixFmtAttrs.AddRange(new[] { WGL_RED_BITS_ARB, value });

            if (attrs.TryGetValue(GLAttribute.GreenSize, out value))
                pixFmtAttrs.AddRange(new[] { WGL_GREEN_BITS_ARB, value });

            if (attrs.TryGetValue(GLAttribute.BlueSize, out value))
                pixFmtAttrs.AddRange(new[] { WGL_BLUE_BITS_ARB, value });

            if (attrs.TryGetValue(GLAttribute.AlphaSize, out value))
                pixFmtAttrs.AddRange(new[] { WGL_ALPHA_BITS_ARB, value });

            if (attrs.TryGetValue(GLAttribute.MultisampleBuffers, out value))
                pixFmtAttrs.AddRange(new[] { WGL_SAMPLE_BUFFERS_ARB, value });

            if (attrs.TryGetValue(GLAttribute.MultisampleSamples, out value))
                pixFmtAttrs.AddRange(new[] { WGL_SAMPLES_ARB, value });

            if (attrs.TryGetValue(GLAttribute.ContextMajorVersion, out value))
                contextAttrs.AddRange(new[] { WGL_CONTEXT_MAJOR_VERSION_ARB, value });

            if (attrs.TryGetValue(GLAttribute.ContextMinorVersion, out value))
                contextAttrs.AddRange(new[] { WGL_CONTEXT_MINOR_VERSION_ARB, value });
            ...
            if (attrs.TryGetValue(GLAttribute.ContextProfileMask, out value))
            {
                switch ((GLContextType) value)
                {
                    case GLContextType.Core:
                        contextAttrs.AddRange(new[] { WGL_CONTEXT_PROFILE_MASK_ARB, 1 });
                        break;
                    case GLContextType.Compatibilty:
                        contextAttrs.AddRange(new[] { WGL_CONTEXT_PROFILE_MASK_ARB, 2 });
                        break;
                    case GLContextType.ES:
                        throw new NotSupportedException("OpenGL ES is not supported on Windows");
                }
            }

            return (pixFmtAttrs.ToArray(), contextAttrs.ToArray());
        }
    }
}
