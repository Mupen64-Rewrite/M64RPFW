...
{
    public static T? GetFunction<T>(nint lib, string name)
    {
        var res = NativeLibrary.GetExport(lib, name);
        return res == nint.Zero ? default : Marshal.GetDelegateForFunctionPointer<T>(res);
    }
    ...
    public static void ResolveDelegate<T>(nint lib, out T del) where T : Delegate
    {
        ...
        if (rtDllImport.Name is null)
        {
            ...
            if (name[0] != 'D') throw new ArgumentException("Type does not match automatic delegate typename");
        ...
        var val = GetFunction<T>(lib, symbol);
        if (val == null)
